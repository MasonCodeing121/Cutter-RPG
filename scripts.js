...
const QUESTS = [
    ...
    {
        id: 3, name: "Stone Breaker",
        desc: "Mine 15 stone from rocks.",
        check: p => p.totalStone >= 15,
        reward: p => { p.money += 100; p.maxHp += 20; p.hp = Math.min(p.hp + 20, p.maxHp); },
        rewardDesc: "$100 + Max HP +20"
    },
    {
        id: 4, name: "The Lumberjack",
        desc: "Chop 50 wood total.",
        check: p => p.totalWood >= 50,
        reward: p => { p.money += 200; },
        rewardDesc: "$200"
    },
    {
        id: 5, name: "Bushmaster",
        desc: "Collect 30 leaves total.",
        check: p => p.totalLeaves >= 30,
        reward: p => { p.money += 150; p.baseSpeed += 25; },
        rewardDesc: "$150 + Speed Up"
    },
    {
        id: 6, name: "Gel Collector",
        desc: "Collect 20 slime gel.",
        check: p => p.totalGel >= 20,
        reward: p => { p.money += 300; },
        rewardDesc: "$300"
    },
    {
        id: 7, name: "Crystal Hunter",
        desc: "Find 3 crystals.\n(Rare drop from rocks!)",
        check: p => p.totalCrystals >= 3,
        reward: p => { p.money += 500; p.maxHp += 25; p.hp = Math.min(p.hp + 25, p.maxHp); },
    ...
    },
    {
        ...
        reward: p => { p.money += 400; p.baseSpeed += 20; },
        rewardDesc: "$400 + Speed Up"
    },
    {
        id: 9, name: "Slime Lord",
        desc: "Kill 30 slimes.",
        check: p => p.kills >= 30,
        reward: p => { p.money += 600; p.maxHp += 30; p.hp = Math.min(p.hp + 30, p.maxHp); },
        rewardDesc: "$600 + Max HP +30"
    },
    {
        id: 10, name: "Master Lumberjack",
        desc: "Chop 200 wood total.",
        check: p => p.totalWood >= 200,
        reward: p => { p.money += 800; p.baseSpeed += 30; },
        rewardDesc: "$800 + Speed Up"
    },
    {
        id: 11, name: "Crystal Collector",
        desc: "Find 10 crystals.\n(Extremely rare!)",
        check: p => p.totalCrystals >= 10,
        reward: p => { p.money += 2000; p.baseSpeed += 25; },
        rewardDesc: "$2000 + Speed Up"
    },
    {
        id: 12, name: "Nature's Champion",
        desc: "200 wood + 100 leaves + 50 gel.",
        check: p => p.totalWood >= 200 && p.totalLeaves >= 100 && p.totalGel >= 50,
        reward: p => { p.money += 2500; p.maxHp += 60; p.hp = Math.min(p.hp + 60, p.maxHp); },
        rewardDesc: "$2500 + Max HP +60"
    },
    {
        id: 13, name: "Slime Exterminator",
        desc: "Kill 100 slimes.\nGood luck.",
        check: p => p.kills >= 100,
        reward: p => { p.money += 3000; p.maxHp += 50; p.hp = Math.min(p.hp + 50, p.maxHp); },
        rewardDesc: "$3000 + Max HP +50"
    },
    {
        id: 14, name: "The Deep Miner",
        desc: "Mine 300 stone total.\nThe rocks await.",
        check: p => p.totalStone >= 300,
        reward: p => { p.money += 3500; p.baseSpeed += 40; },
        rewardDesc: "$3500 + Big Speed Up"
    },
    {
        id: 15, name: "Crystal Legend",
        desc: "Find 30 crystals.\n(Incredibly rare drops!)",
        check: p => p.totalCrystals >= 30,
        reward: p => { p.money += 10000; p.baseSpeed += 50; p.maxHp += 100; p.hp = Math.min(p.hp + 100, p.maxHp); },
...
function animate(currentTime) {
    ...
    if (gameState === "GAME") {
        ...
        drawList.forEach(obj => {
            ...
            if (obj.type === 'tree') ctx.drawImage(images.tree, obj.x - 80 + sX, obj.y - 160, 160, 180);
                ...
                if ((obj.type === 'player' && player.isSwinging) || (obj.type === 'other' && obj.swinging)) {
                    ...
                    let swingAngle = -2.0 + swingProgress * 2.8;
                    ctx.save();
        ...
        // Money + hint
        ctx.fillStyle = "rgba(0,0,0,0.6)"; ctx.fillRect(20, 44, 180, 18);
        ctx.fillStyle = "#ffd54f"; ctx.font = "bold 12px Arial"; ctx.textAlign = "left";
        ctx.fillText(`$${player.money}`, 26, 57);
        ctx.fillStyle = "#9e9e9e"; ctx.font = "11px Arial";
        ctx.fillText("  |  E = Inventory", 26, 57);
        ...
        if (Math.hypot(camera.x - (shopBounds.x + 125), camera.y - (shopBounds.y + 75)) < 180) {
            ...
            ctx.fillStyle = "white"; ctx.fillText("Press E to open Shop", 300, 497);
        }
        if (Math.hypot(camera.x - questGiverPos.x, camera.y - questGiverPos.y) < 150) {
            ctx.fillStyle = "rgba(0,0,0,0.7)"; ctx.fillRect(150, 480, 300, 24);
            ctx.fillStyle = "#ce93d8"; ctx.fillText("Press E to talk to Quest Giver", 300, 497);
        }

        // Quest notification
        if (questNotif && questNotifTimer > 0) {
            let alpha = Math.min(1, questNotifTimer);
            ctx.globalAlpha = alpha;
            ctx.fillStyle = "rgba(123,31,162,0.9)"; ctx.fillRect(100, 160, 400, 50);
            ctx.strokeStyle = "#ce93d8"; ctx.strokeRect(100, 160, 400, 50);
            ctx.fillStyle = "white"; ctx.font = "bold 14px Arial"; ctx.textAlign = "center";
            ctx.fillText("Quest Complete: " + questNotif, 300, 183);
            ctx.fillStyle = "#ffd54f"; ctx.font = "12px Arial";
            ctx.fillText("Talk to the Quest Giver to claim your reward!", 300, 200);
            ctx.globalAlpha = 1;
        }
...
window.addEventListener('keydown', e => {
    ...
    if (e.code === "KeyE" && gameState === "GAME") {
        ...
        if (showQuestGiver) { showQuestGiver = false; return; }
        if (showInventory) { showInventory = false; return; }
        // Open context-sensitive UI
        if (Math.hypot(camera.x - (shopBounds.x + 125), camera.y - (shopBounds.y + 75)) < 180) { showShop = true; return; }
        if (Math.hypot(camera.x - questGiverPos.x, camera.y - questGiverPos.y) < 150) { showQuestGiver = true; questPage = 0; return; }
        // Default: open inventory
        showInventory = true;
    }

    if (e.code === "Escape" && gameState === "GAME") { showShop = false; showQuestGiver = false; showInventory = false; }

    keys[e.code] = true;
    if (["1", "2", "3", "4"].includes(e.key)) selectedSlot = parseInt(e.key) - 1;

    if (e.code === "Space" && gameState === "GAME" && !player.isSwinging && !showShop && !showQuestGiver) {
        if (inventory[selectedSlot].id === 'axe') {
            ...
            mobs.forEach((m, index) => {
                if (Math.hypot(camera.x - m.x, camera.y - m.y) < 100) {
                    m.hp--; m.shake = 10;
                    if (m.hp <= 0) { player.gel++; player.totalGel++; player.kills++; mobs.splice(index, 1); }
                }
            });
            rocks.forEach(r => {
                if (r.hp > 0 && Math.hypot(camera.x - r.x, camera.y - r.y) < 110) {
                    r.hp--; r.shake = 10;
                    if (r.hp <= 0) {
                        let stoneAmt = 2 + Math.floor(Math.random() * 4);
            ...
            crystalNodes.forEach(c => {
                if (c.hp > 0 && Math.hypot(camera.x - c.x, camera.y - c.y) < 110) {
                    c.hp--; c.shake = 10;
                    if (c.hp <= 0) {
                        let crystalAmt = 1 + Math.floor(Math.random() * 2);
                        player.crystals += crystalAmt; player.totalCrystals += crystalAmt;
                    }
                }
            });

            QUESTS.forEach(q => {
                if (!completedQuests.includes(q.id) && q.check(player) && questNotifTimer <= 0) {
                    questNotif = q.name; questNotifTimer = 4;
                }
            });
        }
    }
    ...
    if (showQuestGiver) {
        ...
        visibleQuests.forEach((q, i) => {
            ...
            if (!completedQuests.includes(q.id) && q.check(player)) {
                if (mx > 380 && mx < 510 && my > yBase + 55 && my < yBase + 85) {
                    q.reward(player); completedQuests.push(q.id);
                    questNotif = null; questNotifTimer = 0;
                }
            }
        });

        if (questPage > 0 && mx > 75 && mx < 175 && my > 490 && my < 522) questPage--;
        if ((questPage + 1) * visibleCount < QUESTS.length && mx > 425 && mx < 525 && my > 490 && my < 522) questPage++;
